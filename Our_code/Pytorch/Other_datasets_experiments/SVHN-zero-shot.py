...
def main(n_batches,lr_gen,lr_stud,batch_size,test_batch_size,g_input_dim,ng,ns,test_freq,beta, t_depth, t_width,
    ...
    for i in range(n_batches):
        ...
        if(i % test_freq) == 0:
            ...
            student.train()
            gen_file = './trained_students/SVHN-generator-t' + str(t_depth) + '-' + str(t_width) + '-s-' + str(s_depth) + '-' + str(s_width) + '.pth'
...
student_file = './trained_students/SVHNstud-t-' + str(t_depth) + '-' + str(t_width) + '-student-' + str(s_depth) + '-' + str(s_width) + '.pth'
device = 'cuda:0'
teach_device= 'cuda:0'
