...
def getData(batch_size,test_batch_size,val_percentage):
    ...
    # Download/Load data
    full_training_data = torchvision.datasets.FashionMNIST('/home/test/data',train = True,transform=transform_train,download=True)  
    test_data = torchvision.datasets.FashionMNIST('/home/test/data',train = False,transform=transform_test,download=True)  
...
def main(n_batches,lr_gen,lr_stud,batch_size,test_batch_size,g_input_dim,ng,ns,test_freq,beta, t_depth, t_width,
    ...
    for i in range(n_batches):
        ...
        if(i % test_freq) == 0:
            ...
            gen_file = './trained_students/FashionMNIST-generator-t' + str(t_depth) + '-' + str(t_width) + '-s-' + str(s_depth) + '-' + str(s_width) + '.pth'
...
s_depth = 16
s_width = 1
teacher_file = './Fashion-MNIST-teacher-'+str(t_depth) + '-' + str(t_width) + '.pth'
student_file = './trained_students/Fashion-MNIST-stud-t-' + str(t_depth) + '-' + str(t_width) + '-student-' + str(s_depth) + '-' + str(s_width) + '.pth'
device = 'cuda:0'
teach_device= 'cuda:0'
