...
class FewShotKTSolver(object):
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.args.n_epochs):
            ...
            if epoch % self.args.log_freq == 0 or (self.args.n_epochs - epoch) < 50 :
                test_acc, test_KT_loss = self.test()                
                print('Epoch: ' + str(epoch) + '/' + str(self.args.n_epochs) + ' Train Accuracy: ' + str(round(train_acc*100,6)) + '% Train KT Loss: ' + str(round(train_KT_loss,4)) + ' Time data_time_sec: ' + str(round(data_time,4)) + ' Time batch_time_sec: ' + str(round(batch_time,4)) + ' Test accuracy: ' + str(round(test_acc*100,6)) + '% Test KT_loss: ' + str(round(test_KT_loss,4)) )
